...
template <>
class GetUniqueIdImpl<BackendType::kNccl, Device::Type::kNvidia> {
 public:
  static ReturnStatus Apply(infinicclUniqueId *id) {
    ...
    static_assert(
        sizeof(ncclUniqueId) <= sizeof(id->internal),
        "`infinicclUniqueId::internal` is too small for `ncclUniqueId`");
